...
#define DEFAULT_HOSTNAME  "optimlink.com"
...
const char HTTP_PORTAL_OPTIONS[] PROGMEM  = "<form action=\"/wifi\" method=\"get\"><button class=\"btn\">Configuration</button></form><br/><form action=\"/i\" method=\"get\"><button class=\"btn\">Information</button></form><br/><form action=\"/done\" method=\"get\"><button class=\"btn\">Exit Portal</button></form><br/>";
...
const char HTTP_SAVED[] PROGMEM           = "<div class=\"msg\"><strong>Credentials Saved</strong>.<br><p/>Connecting to WiFi network: <strong>{x}</strong>.<br><br>Please wait for about 10 seconds and then check the network status <a href=\"/\">HERE</a>. <br> <p/></div>";
...
class WiFiManager
{
  public:
    ...
    void          setCaptiveMode(boolean isCaptive);

    boolean       setHostname(const char *hostname);
