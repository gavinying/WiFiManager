...
boolean WiFiManager::autoConnect() {
	String ssid = SSID_PREFIX + String(ESP.getChipId());
...
void WiFiManager::reportStatus(String &page){
	if (WiFi.SSID() != "") {
		if (WiFi.status()==WL_CONNECTED) {
			page += F("Currently <strong>connected</strong> to access point <strong>");
      page += WiFi.SSID();
  		page += F("</strong> on IP <a href=\"http://");
			...
			page += F("Configured to access point <strong>");
      page += WiFi.SSID();
      page += F("</strong>, but currently <strong>not connected</strong> yet.");
		}
	}
	else {
		page += F("No access point currently configured.");
	}
}
...
void WiFiManager::handleInfo() {
	...
	reportStatus(page);
	page += F("<h2>Device Information</h2>");
	page += F("<table class=\"table\">");
	page += F("<thead><tr><th>Name</th><th>Value</th></tr></thead><tbody>");
	page += F("<tr><td>Chip ID</td><td>");
	page += ESP.getChipId();
	page += F("</td></tr>");
